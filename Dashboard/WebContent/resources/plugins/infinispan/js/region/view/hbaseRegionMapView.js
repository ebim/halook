infinispan.HbaseRegionMapView = wgp.AbstractView.extend({
	...
	render : function() {
		...
		new infinispan.hbaseRegionMapAxisStateElementView({
			model : yProperty,
			paper : this.paper
		});
	...
	_getLastTime : function(collectionModels) {
		...
		_.each(collectionModels, function(model, id) {
			var measurementTime = model.get(infinispan.ID.MEASUREMENT_TIME);
